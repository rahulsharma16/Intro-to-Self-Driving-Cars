  ...
  float blurring) 
{
  ...
  for (int i=0;i<height;i++)
  {
    ...
    {
      if(i+dx<0){new_i = (i+dx) + width;}
      else{new_i = (i+dx) % width;}
      if(j+dy<0){new_j = (j+dy) + height;}
      else{new_j = (j+dy) % height;}
      newGrid[int(new_i)][int(new_j)] = row[j];
    }
    row.clear();
  }
	...
	float p_miss) 
{
  ...
  for (int i=0;i<beliefs.size();i++)
  {
    ...
    {
      hit = (color == grid[i][j]);
      temp.push_back(row[j]*(hit*p_hit+(1-hit)*p_miss));
    }
    newGrid.push_back(temp);
    temp.clear();
    row.clear();
