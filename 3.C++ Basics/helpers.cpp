...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
	...
	{adjacent_prob,center_prob,adjacent_prob},{corner_prob,adjacent_prob,corner_prob}};
		...
		for(int j=0;j<grid[0].size();j++)
		{
			...
			for (int dx=-1;dx<2;dx++)
			{
				for(int dy=-1;dy<2;dy++)
				{
					...
					if(i+dy<0){new_i = (i+dy) + grid.size();}
					else{new_i = (i+dy) % grid.size();}
					if(j+dx<0){new_j = (j+dx) + grid[0].size();}
					else{new_j = (j+dx) % grid[0].size();}
